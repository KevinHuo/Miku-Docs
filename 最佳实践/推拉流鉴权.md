...
本文覆盖两部分：
- **推流鉴权**（RTMP Publish）：无校验、限时鉴权（expiry）
- **拉流时间戳防盗链**（Play Anti-Leech）：HLS（.m3u8）、FLV（.flv）、RTMP 播放等

## 术语

- **RTMPPublishDomain**：推流域名（用于 RTMP 推流）  
- **HLS/FLV/RTMP Play Domain**：播放域名（用于拉流播放）  
- **Hub**：直播空间名称  
- **streamTitle**：直播流名称  
...
> Portal 中主/副密钥均可使用；建议使用**副密钥**进行轮换，降低主密钥更换风险。
...
| 协议 | 地址示例 | 用于签名的 `path` |
|---|---|---|
| HLS | `http://play.example.com/hub/stream.m3u8` | `/hub/stream.m3u8` |
| FLV | `http://play.example.com/hub/stream.flv`  | `/hub/stream.flv`  |
| RTMP播放 | `rtmp://play.example.com/hub/stream`  | `/hub/stream`      |
...
sign    = md5("test/testHub/testStream.m3u81761739200").to_lower()
        = e85ab37318c95e8871dcbdc892b8af90
...
> 同一签名方法适用于推流与拉流。区别仅在于：**使用的 Key（PublishKey/PlayKey）** 与 **path**（推流 `/hub/stream`；HLS `/hub/stream.m3u8`；FLV `/hub/stream.flv`）。
...
public class TimestampAntiLeechUrl {
    ...
    public static void main(String[] args) {
        ...
        // HLS 播放（Play）
        String hlsUrl = "http://pili-hls.pilitest.com/testHub/testStream.m3u8";
...
func main() {
	...
	// HLS 播放（Play）
	hls := "http://pili-hls.pilitest.com/testHub/testStream.m3u8"
...
2. **path 必须准确**
   - 推流：`/hub/streamTitle`  
   - HLS：`/hub/streamTitle.m3u8`  
   - FLV：`/hub/streamTitle.flv`  
