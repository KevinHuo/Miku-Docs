...
**推流地址格式**
```
rtmp://<publishDomain>/<hub>/<streamTitle>
```
...
**推流地址格式**
```
[rtmp/http]://<playDomain>/<hub>/<streamTitle>.[flv/m3u8]
```
...
> 同一签名方法适用于推流与拉流。区别仅在于：**使用的 Key（publishKey/playKey）** 与 **path**（推拉流 RTMP `/hub/stream`；拉流 HLS `/hub/stream.m3u8`；拉流 FLV `/hub/stream.flv`）。
...
2. **path 必须准确**
   - 推拉流 RTMP：`/hub/streamTitle`  
   - 拉流 HLS：`/hub/streamTitle.m3u8`  
   - 拉流 FLV：`/hub/streamTitle.flv`  
